...
create_from_template() {
    ...
    if [ ! -f "$template_file" ]; then
        echo "${RED}Error: Template file not found: $template_file${NORMAL}"
        return 1
    fi
    ...
    template_content="${template_content//\{\{CHARACTER_NAME\}\}/$char_name}"
    template_content="${template_content//\{\{CHARACTER_KEBAB\}\}/$char_kebab}"
    template_content="${template_content//\{\{CHARACTER_KEBAB_UPPER\}\}/$(to_upper_case_with_dashes "$char_kebab")}"
    template_content="${template_content//\{\{CREATOR_ID\}\}/$CREATOR_ID}"
    template_content="${template_content//\{\{CREATED_DATE\}\}/$CREATED_DATE}"
    
    # Write to output file
    echo "$template_content" > "$output_file"
    echo "${GREEN}Created file:${NORMAL} $output_file"
}

# Create a basic file with standard header
create_file() {
    local file_path="$1"
    local title="$2"
    local section="$3"
...
create_primary_character() {
    ...
    # Create from templates where available
    create_from_template "${TEMPLATE_DIR}/character-overview-brief.template" \
                       "${base_dir}/_profile/character-${kebab_name}-overview-brief.md" \
                       "$proper_name" "$kebab_name"
                       
    create_from_template "${TEMPLATE_DIR}/character-attributes-physical.template" \
                       "${base_dir}/_profile/attributes/character-${kebab_name}-attributes-physical.md" \
                       "$proper_name" "$kebab_name"
                       
    create_from_template "${TEMPLATE_DIR}/character-attributes-personality.template" \
                       "${base_dir}/_profile/attributes/character-${kebab_name}-attributes-personality.md" \
                       "$proper_name" "$kebab_name"
    
    # Create standard files for other sections
...
create_minor_character() {
    ...
    local proper_name=$(to_proper_case "$char_name")
    local base_dir="${CHARACTER_DIR}/minor-characters/${kebab_category}/${kebab_name}"
    ...
    # Create files
    create_file "${base_dir}/character-${kebab_name}-overview.md" \
                "$proper_name - Overview" \
                "Character Summary" \
    ...
    create_file "${base_dir}/relationship-${kebab_name}-fred-turd-primary.md" \
